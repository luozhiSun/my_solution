...
class Environment(tk.Tk, object):
    def __init__(self):
        ...
        self.title('RL Q-learning-baseline')
    ...
    def build_environment(self):
        ...
        """添加环境中的实线，初步设定为每5个单位显示线段"""
        for column in range(0, env_width * pixels, 5*pixels):
            x0, y0, x1, y1 = column, 0, column, env_height * pixels
            self.canvas_widget.create_line(x0, y0, x1, y1, fill='grey')
        for row in range(0, env_height * pixels, 5*pixels):
        ...
        # Obstacle 1
        # Defining the center of obstacle [30,50]
        ...
        self.obstacle1 = self.canvas_widget.create_rectangle(
            ...
            obstacle1_center[0] + 50, obstacle1_center[1] + 50,  # Bottom right corner
             fill='#00BFFF')
        print(self.obstacle1)
        # Saving the coordinates of obstacle 1 according to the size of agent
        # In order to fit the coordinates of the agent
        """根据agent的大小保存障碍物的坐标，以适应agent的坐标"""
